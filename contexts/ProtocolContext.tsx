...
import { setConsoleWarningsSuppressed } from '@/utils/logger';
...
const STORAGE_KEYS = {
  ...
  HIDE_CONSOLE_WARNINGS: '@protocol_hide_console_warnings',
...
export const [ProtocolProvider, useProtocol] = createContextHook<ProtocolContextValue>(() => {
    ...
    const loadSettings = async () => {
      try {
        const [
          ...
          AsyncStorage.getItem(STORAGE_KEYS.HIDE_CONSOLE_WARNINGS),
        ...
        if (hideWarnings !== null) setHideConsoleWarningsState(hideWarnings === 'true');
        if (activeProto) {
  ...
  useEffect(() => {
    setConsoleWarningsSuppressed(hideConsoleWarnings);
  }, [hideConsoleWarnings]);
  ...
  const setHideConsoleWarnings = useCallback(async (hidden: boolean) => {
    ...
    await AsyncStorage.setItem(STORAGE_KEYS.HIDE_CONSOLE_WARNINGS, String(hidden));
  }, []);

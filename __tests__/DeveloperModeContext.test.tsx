...
describe('DeveloperModeContext', () => {
  ...
  test('defaults match expected values', async () => {
    ...
    // DeveloperModeContext migrates plaintext PINs to hashed storage on load.
    expect(ctxRef!.developerMode.pinCode).toMatch(/^sha256:/);
    await expect(ctxRef!.verifyPinCode(DEFAULT_DEVELOPER_MODE.pinCode || '')).resolves.toBe(true);
  });
